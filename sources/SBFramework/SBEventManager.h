...
private:
	...
	private:
		std::list< EventReceiver<EventType>* > m_eventReceivers;
	};
...
{
	for (typename std::list< EventReceiver<EventType>* >::iterator it = m_eventReceivers.begin(); it != m_eventReceivers.end(); ++it)
	{
...
{
	for (typename std::list< EventReceiver<EventType>* >::iterator it = m_eventReceivers.begin(); it != m_eventReceivers.end(); )
	{
		typename std::list< EventReceiver<EventType>* >::iterator toErase = it++;
		if ((*toErase) == receiver)
		{
			m_eventReceivers.erase(toErase);
		}
	}
}
